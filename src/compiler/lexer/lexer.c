...
Token* lexer_get_next_token(Lexer* lexer) {
    ...
    if (_is_letter(lexer->ch)) {
        ...
        token = token_new(_get_token_kind_from_literal(ident_literal, len), ident_literal);

        return token;
    } else if(_is_digit(lexer->ch)) {
        token = token_new(TOKEN_KIND_INT, _lexer_read_int(lexer));
